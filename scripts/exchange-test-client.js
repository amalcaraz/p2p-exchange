...
function summariseBook(book) {
  ...
  return {
    bids: count(book.bids),
    bidsVol: volume(book.bids),
    asks: count(book.asks),
    asksVol: volume(book.asks),
  };
}

async function main() {
  console.log(`[test] discovering live nodes on ${GRAPE_URL}...`);
  const nodeIds = await discoverNodes({
    grapeUrl: GRAPE_URL,
    serviceName: PEER_SERVICE,
  });
  if (nodeIds.length === 0) {
    console.error(
      '[test] no nodes discovered — start the cluster first (npm run dev:grapes && npm run dev:cluster -- 3)'
    );
    process.exit(1);
  }
  ...
  console.log(
    `[test] submitting ${ORDER_COUNT} random orders in ${waves} wave(s) of up to ${WAVE_SIZE} (via ${PEER_SERVICE})...`
  );
  const t0 = Date.now();
  let ok = 0,
    failed = 0,
  ...
  for (let w = 0; w < waves; w++) {
    ...
    for (const r of results) {
      if (r.status === 'fulfilled') {
        ok++;
        totalTrades += r.value?.trades?.length ?? 0;
      } else {
        failed++;
        if (!firstFailure) firstFailure = r.reason?.message;
      }
    }
    submitted += batch;
  ...
  console.log(
    `[test] ${ok}/${ORDER_COUNT} ok, ${failed} failed, ${totalTrades} trades, ${elapsed}ms`
  );
  ...
  console.log(
    `[test] convergence across ${nodeIds.length} nodes: ${allMatch ? 'PASS' : 'FAIL'}`
  );
  for (let i = 0; i < nodeIds.length; i++) {
    const s = summariseBook(books[i]);
    const match = JSON.stringify(books[i]) === ref;
    console.log(
      `  node ${nodeIds[i]}  bids=${s.bids} (vol ${s.bidsVol})  asks=${s.asks} (vol ${s.asksVol})  ${match ? '' : 'DIVERGED'}`
    );
  }
