...
function spawnGrapes() {
  ...
  const g1 = spawn(process.execPath, [grapeBin, '--dp', '20001', '--aph', '30001', '--bn', '127.0.0.1:20002', '--dpa', '10000'], { stdio: 'pipe', cwd: ROOT });
  const g2 = spawn(process.execPath, [grapeBin, '--dp', '20002', '--aph', '40001', '--bn', '127.0.0.1:20001', '--dpa', '10000'], { stdio: 'pipe', cwd: ROOT });
  return [g1, g2];
}

function spawnNode(extraEnv = {}) {
  return spawn(process.execPath, ['--env-file=.env', path.resolve(ROOT, 'src/exchange/index.js')], {
...
async function waitForReady(proc, timeoutMs = 8000) {
  ...
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => reject(new Error('node did not become READY in time')), timeoutMs);
  });
}
...
async function discoverNodeIds(timeout = 3000) {
  ...
  await client.start();
  try {
    const { results } = await client.broadcast('exchange_peer', { action: 'whoami' }, { timeout });
...
test('3-node convergence under concurrent load (60 random orders in 3 waves)', { timeout: 120_000 }, async (t) => {
  const NODE_COUNT = 3;
  const WAVES = 3;
  ...
  const submit = (o) => client.request('exchange_peer', { action: 'submitOrder', ...o }, 30_000);
  ...
  for (let w = 0; w < WAVES; w++) {
    ...
    assert.equal(failed.length, 0, `wave ${w}: ${failed.length} failed; first: ${failed[0]?.reason?.message}`);
    totalTrades += results.reduce((s, r) => s + (r.value?.trades?.length ?? 0), 0);
    await sleep(500);
  }
  ...
  const books = await Promise.all(
    nodeIds.map((id) => client.request(`exchange_rpc_${id}`, { action: 'getBook' }, 15_000)),
  );
  for (let i = 1; i < books.length; i++) {
    assert.deepEqual(books[i], books[0], `book from node ${nodeIds[i]} diverges from node ${nodeIds[0]}`);
  }

  assert.ok(totalTrades > 0, 'expected at least one trade across 60 random orders');
});

test('evicted-then-resumed node resyncs from a live peer', { timeout: 60_000 }, async (t) => {
  ...
  const nodes = [spawnNode(SHORT_EVICTION), spawnNode(SHORT_EVICTION), spawnNode(SHORT_EVICTION)];
  t.after(() => killAll(nodes));
  await Promise.all(nodes.map((n) => waitForReady(n, 12_000)));
  ...
  assert.ok(liveIds.length >= 2, `need >=2 live node ids while one is frozen, got ${liveIds.length}: ${liveIds.join(',')}`);
  const liveSubmit = (id, o) => client.request(`exchange_rpc_${id}`, { action: 'submitOrder', ...o }, 30_000);
  await Promise.all([
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    if (allIds.length >= 3) {
      books = await Promise.all(
        allIds.map((id) => client.request(`exchange_rpc_${id}`, { action: 'getBook' }, 15_000)),
      );
  ...
  assert.ok(allIds.length >= 3, `expected 3 live node ids after resume, got ${allIds.length}: ${allIds?.join(',')}`);
  for (let i = 1; i < books.length; i++) {
    assert.deepEqual(books[i], books[0], `book from ${allIds[i]} diverges from ${allIds[0]} after resync`);
  }
});

test('late-joining node gets a snapshot and converges', { timeout: 40_000 }, async (t) => {
  const grapes = spawnGrapes();
  t.after(() => killAll(grapes));
  await sleep(800);
  ...
  const submit = (o) => client.request('exchange_peer', { action: 'submitOrder', ...o });
  await Promise.all([
    submit({ side: 'sell', price: 100, amount: 5 }),
  ...
  const books = [];
  for (let i = 0; i < 10; i++) books.push(await client.request('exchange_peer', { action: 'getBook' }));
  for (let i = 0; i < books.length; i++) {
    assert.deepEqual(books[i], pre, `book #${i} diverges from the pre-join snapshot`);
  }
  ...
  await sleep(1000);
  const after = [];
  for (let i = 0; i < 10; i++) after.push(await client.request('exchange_peer', { action: 'getBook' }));
