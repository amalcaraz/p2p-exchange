...
class ExchangeNode {
  ...
  async start() {
    ...
    this._server = new GrenacheServer({ grapeUrl: this._cfg.GRAPE_URL, port: this._rpcPort });
    await this._server.start();
    this._server.announce(this._cfg.PEER_SERVICE);
    this._server.announce(`${this._cfg.RPC_SERVICE_PREFIX}_${this._cfg.SELF_ID}`);
    ...
    this._exchange = new Exchange({
      ...
      fetchSnapshot: (peerId) => this._rpcClient.request(
    ...
    this._server.onRequest((rid, key, payload, handler) => {
      ...
      if (action === 'submitOrder') return handlers.submitOrder(rid, key, payload, handler);
      if (action === 'getBook') return handlers.getBook(rid, key, payload, handler);
      if (action === 'getSnapshot') return handlers.getSnapshot(rid, key, payload, handler);
      if (action === 'peerEvent') return handlers.peerEvent(rid, key, payload, handler);
      if (action === 'whoami') return handlers.whoami(rid, key, payload, handler);
    ...
    console.log(`[exchange] node ${this._cfg.SELF_ID} READY (RPC port ${this._rpcPort})`);
  }
...
try {
  ...
  console.error('[exchange] failed to start:', err); process.exit(1);
}
