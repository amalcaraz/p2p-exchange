...
class ExchangeHandlers {
  ...
  async submitOrder(_rid, _key, payload, handler) {
    try {
      ...
      if (side !== 'buy' && side !== 'sell')
        throw new ValidationError('side must be buy|sell');
  ...
  async getBook(_rid, _key, _payload, handler) {
    try {
      handler.reply(null, this._node.getBook());
    } catch (err) {
      handler.reply(err, null);
    }
  }
  ...
  async getSnapshot(_rid, _key, _payload, handler) {
    try {
      handler.reply(null, await this._node.getSnapshot());
    } catch (err) {
      handler.reply(err, null);
    }
  }
  ...
  async whoami(_rid, _key, _payload, handler) {
    try {
      handler.reply(null, { nodeId: this._node.selfId });
    } catch (err) {
      handler.reply(err, null);
    }
  }
